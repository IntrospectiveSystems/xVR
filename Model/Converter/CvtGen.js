...
(function CvtGen() {
	...
	function Convert(com, fun) {
		...
		zip.generateAsync({type:'nodebuffer'}).then(function(buf) {
			log.v('Buf length', buf.length);
			that.addModule(com.Name, buf, function(err, name) {
				log.v('Back from addModule');
				if(err) {
					log.e(' ** addModule Err:' + err);
					if(fun)
						fun('addModeuled Err:' + err);
					return;
				}
				if(name)
					log.v('Module name:' + name);
				let mod = {};
				mod.Module = name;
				mod.Par = {};
				that.genModule(mod, function(err, pid) {
					if(err) {
						log.e(' ** genModule Err:' + err);
						if(fun)
							fun('genModule Err:' + err);
						return;
					}
					let q = {};
					q.Cmd = 'Generate';
					that.send(q, pid, function(err, r) {
						if(err) {
							log.e(' ** Generate Err:' + err);
							if(fun)
								fun('Generate Err:' + err);
							return;
						}
						log.v('Generate succeeded');
						if('X3D' in r)
							com.X3D = r.X3D;
						if(fun) {
							fun(null, com);
